...
class gitlab (
  ...
  if ($db_adapter or $db_encoding or $db_database or $db_pool or $db_username
    ...
    and ($postgresql_enable == undef or $postgresql_enable) {
    fail('db_adapter, db_encoding, db_database, db_pool, db_username, db_password, db_host, and db_socket cannot be set unless postgresql_enable is false')
  }
  ...
  if ($ci_db_adapter or $ci_db_encoding or $ci_db_database or $ci_db_pool or $ci_db_username
    ...
    fail('ci_db_adapter, ci_db_encoding, ci_db_database, ci_db_pool, ci_db_username, ci_db_password, ci_db_host, and ci_db_socket cannot be set unless postgresql_enable is false or mysql_enable is true')
  }
